...
AC.Detector = {
	...
	// ** {{{ AC.Detector.hasGyro() }}} **
	// 
	// Returns whether the device has a gyroscope, or null if we can't tell yet.
	//
	// We need to listen to the gyro and have it give us data to tell whether
	// or not it exists, so use the following chart to determine if the device
	// has a gyro or not:
	//
	// If {{{AC.Detector.hasGyro}}} returns:
	// * {{{true}}}: The device has a gyro
	// * {{{false}}}: The device probably does not have a gyro
	// * {{{null}}}: We can't tell yet whether the device has a gyro, but we're
	//   going to test it now. Re-run {{{AC.Detector.hasGyro}}} until it returns true or false;
	//
	// Documentation for DeviceOrientationEvent can be found here:
	...
	hasGyro: function () {
		...
		// If we're on a device running iOS 4.2 or later, this will be true
		if ('DeviceOrientationEvent' in window && window.DeviceOrientationEvent !== null) {
			if(this._testingForGyro === false){
				...
				// We know the software can handle a gyro, butneed to test to make sure that the
				// hardware actually has a gyro.
				var self = this;
				this.boundTestingForGyro = function(e){
					self.testingForGyro(e);
				};
				window.addEventListener("deviceorientation", this.boundTestingForGyro, true);

				// If we don't hear anything back within 250s, then the device probably doesn't
				// have a gyro. The average time to get a positive result is <10ms for 4.3+, <30ms for 4.2-
	...
	// ** {{{ AC.Detector.isiPadWithGyro() }}} **
	// 
	// Returns whether the device has a gyroscope and is an iPad, or null if we can't tell yet.
	//
	// **NOTE**: The first generation iPad does not have a gyro, but the iPad 2 does. This test
	// can be used to distinguish between the two devices.
	//
